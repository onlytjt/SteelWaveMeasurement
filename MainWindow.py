...
class MainUI(QMainWindow, Ui_MainWindow):
    ...
    def initVariable(self):
        ...
        self.ROIRANGE = 300
        self.rotatePeriod = 127  # 旋转一周采集的图像数
    ...
    def displayOriImg(self, oriImg):  # 原始图像区域的处理
        ...
        img = img.scaled(1226, 150, Qt.KeepAspectRatio)  # 从1226*150以0.8倍变换得来
    ...
    def displayEdgeImg(self, oriImg):  # 边缘显示区域的处理
        ...
        self.edit_big_height.setText(QString(str(waveHeight)))
        self.edit_big_length.setText(QString(str(waveLength)))  # 实时显示波高像素值和直径像素值

        if (not self.isMeasuring) or (not ip.doHoughTrans()):  # 检测图像没有钢丝。或系统刚启动，没在测量时
            # 边缘显示区域也显示原始图像
            img = q2n.gray2qimage(oriImg)
            img = img.scaled(1226, 150, Qt.KeepAspectRatio)
            self.label_canny.setPixmap(QPixmap.fromImage(img))  # 在原始图像显示区域写入图像
            return
        else:  # 有钢丝情况，显示Canny算子计算后的边缘
            img = cv2.resize(ip.cannyImg, (1226, 150), cv2.INTER_NEAREST)
    ...
    def dealWaveHeightList(self, data):
        extremumList = []
        extremumBigWave = []
        extremumSmallWave = []
        indexOfExtremum = []  # 用于保存数据中找到的极值的索引值
        # 首先做前后向差分，找到一些极值点
        for index, value in enumerate(data):
            right1 = 0 if (index+1)>self.rotatePeriod else index+1  # 保证下标右侧循环，保证不会溢出
            ...
            if value>=data[index-1] and value>=data[index-2] and value>=data[right1] and value>=data[right2]:
                if 1 < index < self.rotatePeriod-1:
                    extremumList.append(value)
                    indexOfExtremum.append(index)
        # 去除极值中索引相邻，值重复的项
        for i, index in enumerate(indexOfExtremum):
            print index, ": ", data[index]
            if index-1 == indexOfExtremum[i-1] and data[index] == data[index-1]:
                del indexOfExtremum[i]  # 保留较小的下标值
        # 直接利用波高极值的平均值进行分类，后面其他算法再讨论
        for height in extremumList:
            if height > np.array(extremumList).mean():
                extremumBigWave.append(height)
            else:
                extremumSmallWave.append(height)
        print extremumSmallWave, extremumBigWave
        smallWaveHeight = np.array(extremumSmallWave).mean()
        bigWaveHeight = np.array(extremumBigWave).mean()
    ...
    def completeMeasure(self):
        ...
        smallWaveHeightPixel, bigWaveHeightPixel = self.dealWaveHeightList(self.waveHeightList)
